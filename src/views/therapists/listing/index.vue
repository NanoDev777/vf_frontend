...
import { useSelectedTherapistStore } from '@/stores/therapist';
...
import FormTherapist from '../components/form.vue';

const loaderStore = useLoaderStore();
const snackbarStore = useSnackbarStore();
const selectedTherapistStore = useSelectedTherapistStore();
...
const headers = ref([
  ...
  { title: 'Especialidad', key: 'profession'},
...
const selected = ref(selectedTherapistStore.selectedTherapist);
...
const therapistToEdit = ref(null);
...
const loadItems = async ({ page, itemsPerPage, sortBy }) => {
  ...
  try {
    ...
    const response = await TherapistService.listTherapists({ page, itemsPerPage, sort, filter });
...
watch(selected, (newValue) => {
  selectedTherapistStore.setSelectedTherapist(newValue);
  nextTick(() => {
    updateRowStyles();
  });
}, { deep: true });
...
const showEditDialog = async (item) => {
  try {
    ...
    const response = await TherapistService.editTherapist(`therapists/${item.id}/edit`);
    if (response.status == 200) {
      therapistToEdit.value = response.data.data;
...
const deleteItem = async () => {
  try {
    ...
    const response = await TherapistService.deleteTherapist([idItemDelete.value]);
    if (response.status == 200) {
...
const deleteItems = async () => {
  try {
    ...
    const response = await TherapistService.deleteTherapist(items);
    if (response.status == 200) {
      loadingSelectedDialog.value = false;
      isDialogSelectedVisible.value = false;
      selected.value = [];
      selectedTherapistStore.clearSelectedTherapist();
      snackbarStore.showSnackbar(response.data.message);
      reloadTable();
      if (response.data.data.length > 0) {
        push.info('Algunos resgistros no se eliminaron');
...
const clearSelectedTherapist = () => {
  selected.value = []
  selectedTherapistStore.clearSelectedTherapist();
}

const goToTherapistDeatil = (id) => {
  router.push({ name: 'therapist-detail', params: { id } })
}
</script>
<template>
  <VCard title="LISTA DE TERAPISTAS">
    <template #append>
      <div class="pr-2">
        ...
        >
          <template #top>
            ...
            <v-sheet class="pa-2 mb-2" elevation="1" color="#F4F6FF">
              <div class="demo-space-x">
                ...
                <VBtn @click="clearSelectedTherapist" title="Quitar Seleccionados" variant="flat" color="secondary">
                  <VIcon
          ...
          <template #item.actions="{ item }">
            <div class="d-flex gap-1">
              <IconBtn @click="goToTherapistDeatil(item.id)">
  ...
  <!-- Componente FormTherapist -->
  <FormTherapist :isDialogTherapistVisible="isDialogVisible" :therapist="therapistToEdit" :title="'Registrar Terapista'" @update:isDialogTherapistVisible="handleDialogClose" />
