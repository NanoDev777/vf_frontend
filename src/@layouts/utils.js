...
export const isNavLinkActive = (link, router) => {
  ...
  // Check if any matched route's meta section matches
  const currentSection = router.currentRoute.value.meta.subject

  return matchedRoutes.some(route => {
    return (
      route.name === resolveRoutedName ||
      route.meta.navActiveLink === resolveRoutedName ||
      (currentSection && link.subject === currentSection)
    )
  })
}
